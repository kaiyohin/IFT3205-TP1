...
void filter(float ** MatriceImgR, float ** MatriceImgI, int length, int width) {
    ...
    for (int i=0; i<length; i++) {
        for (int j=0; j<width; j++) {
                MatriceImgR[i][j]=MatriceImgR[i][j]*sqrtf(powf(i-center_length, 2) + powf(j-center_width, 2));
                MatriceImgI[i][j]=MatriceImgI[i][j]*sqrtf(powf(i-center_length, 2) + powf(j-center_width, 2));
        }
    }
}
